  ...
  <main>
    <section class="hero">
      <div class="hero-content">
        <div class="notice-card">
          <div class="notice-head"><span class="notice-badge">公告</span><span class="notice-date">{{ notice.updated_at || '-' }}</span></div>
    ...
    <section class="subjects-section" v-if="!selectedSubject && !query">
      <div class="container">
        ...
        <div class="nav-search" style="max-width:520px;margin-bottom:18px">
        ...
        <div class="subjects-grid">
          <button v-for="s in subjects" :key="s.name" class="subject-card" @click="openSubject(s.name)">
            ...
            <div class="subject-card-tags">
              <span v-for="(_, ext) in s.extensions" :key="ext" class="subject-card-tag">{{ ext.replace('.', '') }}</span>
            </div>
          </button>
        </div>
      </div>
    ...
    <section class="files-section" v-else>
      <div class="container">
        ...
        <div class="folders-row" v-if="folders.length">
          ...
          <button v-for="f in folders" :key="f.path" class="folder-chip" :class="{active: subCategory === f.path}" @click="setFolder(f.path)">{{ f.path }} <span class="folder-chip-count">{{ f.file_count }}</span></button>
        </div>
        <div class="file-table-wrap">
          <table class="file-table">
            <thead><tr><th>文件名</th><th>学科</th><th>大小</th><th>日期</th><th></th></tr></thead>
            <tbody>
              <tr v-for="f in files" :key="f.id">
                <td><div class="file-name-cell"><span class="file-ext-badge">{{ f.extension.replace('.', '').toUpperCase() }}</span><span>{{ f.original_name }}</span></div></td>
                <td>{{ f.category }}<span v-if="f.sub_category">/{{ f.sub_category }}</span></td>
                <td>{{ f.file_size }}</td>
